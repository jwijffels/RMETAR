names_result <- c(
  "characters","mdspbool","integers","numerics",
...
names_char_single <- c(
  "synoptic_cloud_type", "snow_depth_group", "codeName", "stnid", "horiz_vsby", "dir_min_horiz_vsby", 
...
names_mdspbool <- c(
  "Indeterminant3_6HrPrecip", "Indeterminant_24HrPrecip", "CIGNO", "SLPNO", "ACFTMSHP", "NOSPECI", "FIRST", "LAST", 
...
names_integers <- c(
  "TornadicDistance", "ob_hour", "ob_minute", "ob_date", "minWnDir", "maxWnDir", "VertVsby", "temp", 
...
names_numerics <- c(
  "SectorVsby", "WaterEquivSnow", "VSBY_2ndSite", "prevail_vsbySM", "prevail_vsbyM", "prevail_vsbyKM", 
...
decodemetar <- function(x, printdecodedmetar = FALSE){
  ...
  names(result$characters)        <- tolower(names_characters)
  names(result$characters$single) <- tolower(names_char_single)
  names(result$mdspbool)          <- tolower(names_mdspbool)
  names(result$integers)          <- tolower(names_integers)
  names(result$numerics)          <- tolower(names_numerics)
